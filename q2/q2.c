...
void parallel(char *string){
    ...
    int start_position = (STRINGSIZE / (n + 2)) * 2;
    ...
    // }
    if (current_thread_num != 0)//optimal thread should have more work to do 
    {                           //so I did not evenly seprate the string 
        t = 4;
        current_state = 1;
        iteretion = STRINGSIZE / (n + 2);
        detect_position += (start_position + (iteretion)*(current_thread_num-1));
        }
    else
    {
        iteretion = (STRINGSIZE / (n + 2)) * 2;
    }
...
int main(int argc, char *argv[]) {
    ...
    initTable();
    omp_set_num_threads(n+1);//set number of thread
    int initTable[n+1][5];
...
#pragma omp parallel//parallel part
    parallel(string);
#pragma omp barrier
